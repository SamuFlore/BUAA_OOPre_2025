...
大模型工具能够有效提高生产，学习效率。但是《面向对象程序先导》课程教学的不只是Java编程能力，更是面向对象的思想，这是大模型无法给同学们的。无节制的滥用大模型会严重影响同学的学习效果和课程分数的公平性。为规范大模型使用，制订以下大模型使用规范：

课程团队在以下情况下允许并鼓励同学们使用大模型：

- 使用大模型辅助正向建模
- 使用大模型进行局部代码补全（copilot等工具）
- 使用大模型搭建评测机
- 使用大模型辅助 debug 等工作
- 使用大模型学习 Java 语法，设计模式等知识
- 使用大模型辅助理解指导书的迭代内容
- 其他有助于同学们锻炼能力的场景
...
同样，对于大模型的使用，我们会对你的作业进行扫描。对于可疑度较高的作业会进行人工复核。如果复核后确认了某位同学的一次作业大部分由大模型完成，我们会做出相应的惩罚，包括但不限于扣除本次作业部分分数，取消当次作业成绩等。
...
|   周二 17:00   |                      发布本周作业                      |
|   周四 12:00   |                    开启本周作业中测                    |
| 次周周一 20:00 |         公测提交截止，系统自动开始本周作业强测         |
| 次周周二 12:00 |                  公布本周作业强测结果                  |
| 次周周二 16:00 | 开始本周作业bug修复（持续一周）；上一周作业bug修复截止 |
