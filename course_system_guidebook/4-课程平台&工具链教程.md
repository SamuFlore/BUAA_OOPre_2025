...
```
git init
git remote add origin git@gitlab.oo.buaa.edu.cn:oo_2025_homeworks/xxx.git
```

如果你并没有创建文件夹，你可以进入输入下面的指令来克隆远程文件夹，这会帮你直接建立好git存储库

```
git clone git@gitlab.oo.buaa.edu.cn:oo_2025_homeworks/xxx.git
```
...
完成其他任务后，点击最下方的“提交回答”按钮即可提交对应的代码。另外，为了保证评测安全与公正，所有提交的代码（包括src和test目录下的代码）请不要使用涉及系统操作、网络通信、反射、输出重定向等可能被用于绕过评测、作弊或影响评测结果的Java库、类或关键字，否则作业将被判为无效。
