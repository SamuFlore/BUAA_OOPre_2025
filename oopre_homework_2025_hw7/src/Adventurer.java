import java.util.ArrayDeque;
import java.util.ArrayList;

public class Adventurer implements Master, Servant {
    private String id;
    private int equippedBottlesNum;
    private ArrayList<Bottle> bottles; //item
    private ArrayDeque<Bottle> equippedBottles;
    private ArrayList<Equipment> equipments; //item
    private ArrayList<Spell> spells;
    private ArrayList<Adventurer> servants; // 全体下级
    private ArrayList<Adventurer> masters; // 全体上级
    private int hitPoint; //HP
    private int atk;
    private int def;
    private int mana; //MP
    private int money;
    private Equipment myWeapon;
    private Equipment myArmour;

    public Adventurer(String id) {
        this.id = id;
        this.equippedBottlesNum = 0;
        this.bottles = new ArrayList<>();
        this.equippedBottles = new ArrayDeque<>();
        this.equipments = new ArrayList<>();
        this.spells = new ArrayList<>();
        this.servants = new ArrayList<>();
        this.masters = new ArrayList<>();
        this.hitPoint = 500;
        this.atk = 1;
        this.def = 0;
        this.mana = 10;
        this.money = 50;
        this.myArmour = null;
        this.myWeapon = null;
    }

    public Equipment getWeapon() { return this.myWeapon; }

    public String getID() { return id; }

    public int getHitPoint() { return hitPoint; }

    public int getAtk() { return atk; }

    public int getDef() { return def; }

    public int getTotalDef() {
        if (this.myArmour == null) { return getDef(); }
        else { return getDef() + this.myArmour.getCe(); }
    }

    public int getMana() { return mana; }

    public int getMoney() { return money; }

    public ArrayList<Adventurer> getServants() { return this.servants; }

    public ArrayList<Adventurer> getMasters() { return this.masters; }

    public void addHitPoint(int value) { this.hitPoint += value; }

    public void addAtk(int value) { this.atk += value; }

    public void addDef(int value) { this.def += value; }

    public void addMana(int value) { this.mana += value; }

    public void addMoney(int value) { this.money += value; }

    public void removeBottle(Bottle bottle) { //使用了bottle
        this.bottles.remove(bottle);
        this.equippedBottles.remove(bottle);
        this.equippedBottlesNum--;
    }

    public void underAttack(int value) {
        this.hitPoint -= value;
        if (this.hitPoint <= 0) {
            this.hitPoint = 0;
            for (Adventurer master : this.masters) {
                master.getServants().remove(this); // 自己的全部上级的下级移除自己
                for (Adventurer serv : this.getServants()) {
                    master.getServants().remove(serv); // 自己的全部上级的下级移除自己的全部下级
                }
            }
            for (Adventurer servant : this.servants) {
                servant.getMasters().remove(this); // 自己的全部下级的上级移除自己
                for (Adventurer mast : this.masters) {
                    servant.getMasters().remove(mast); // 自己的全部下级的上级移除自己的全部上级
                }
            }
            this.getMasters().clear();
            this.getServants().clear();
        }
    }

    public void checkHelp(int formerHitPoint) {
        if (this.getHitPoint() <= Math.floorDiv(formerHitPoint, 2)) {
            this.helpMe(); // 寻求支援
        }
    }

    public void addBt(Bottle bottle) {
        if (this.hitPoint == 0) {
            System.out.printf("%s is dead!\n", this.id);
        }
        else {
            this.bottles.add(bottle);
        }
    }

    public void addEqpmt(Equipment equipment) {
        if (this.hitPoint == 0) {
            System.out.printf("%s is dead!\n", this.id);
        }
        else {
            this.equipments.add(equipment);
        }
    }

    public void addSp(Spell spell) {
        if (this.hitPoint == 0) {
            System.out.printf("%s is dead!\n", this.id);
        }
        else {
            this.spells.add(spell);
        }
    }

    public void rmItem(String itemID) {
        if (this.hitPoint == 0) {
            System.out.printf("%s is dead!\n", this.id);
        }
        else {
            Bottle foundBottle = Factory.fdBottle(this.bottles, itemID);
            if (foundBottle != null) {
                if (this.equippedBottles.remove(foundBottle)) {
                    this.equippedBottlesNum--;
                }
                String className = foundBottle.getClassName();
                System.out.printf("%s\n", className);
                this.bottles.remove(foundBottle);
                return;
            }
            Equipment foundEquipment = Factory.findEquipment(this.equipments, itemID);
            if (foundEquipment != null) {
                if (this.myArmour != null && this.myArmour.equals(foundEquipment)) {
                    this.myArmour = null;
                }
                else if (this.myWeapon != null && this.myWeapon.equals(foundEquipment)) {
                    this.myWeapon = null;
                }
                String className = foundEquipment.getClassName();
                System.out.printf("%s\n", className);
                this.equipments.remove(foundEquipment);
                return;
            }
        }
    }

    public void takeItem(String itemID) {
        if (this.hitPoint == 0) {
            System.out.printf("%s is dead!\n", this.id);
        }
        else {
            Bottle foundBottle = Factory.fdBottle(this.bottles, itemID);
            if (foundBottle != null) {
                String className = foundBottle.getClassName();
                System.out.printf("%s\n", className);
                if (this.equippedBottlesNum == 10) {
                    this.equippedBottles.removeFirst();
                    this.equippedBottles.addLast(foundBottle);
                }
                else {
                    this.equippedBottles.addLast(foundBottle);
                    this.equippedBottlesNum++;
                }
                foundBottle.setIsEquipped();
                return;
            }
            Equipment foundEquipment = Factory.findEquipment(this.equipments, itemID);
            if (foundEquipment != null) {
                String className = foundEquipment.getClassName();
                System.out.printf("%s\n", className);
                if (className.equals("Armour")) { this.myArmour = foundEquipment; }
                else if (className.equals("Sword") || className.equals("Magicbook")) {
                    this.myWeapon =  foundEquipment;
                }
                return;
            }
        }
    }

    public Usable fdUsable(String usableId) { //找寻可用物品
        Bottle foundBottle = Factory.fdEquippedBottle(this.equippedBottles, usableId);
        if (foundBottle != null) { return foundBottle; }

        Spell foundSpell = Factory.fdSpell(this.spells, usableId);
        if (foundSpell != null) { return foundSpell; }

        return null;
    }

    public Bottle fdBottle(String  bottleId) { // 找不在背包中的药水
        for (Bottle bottle : this.bottles) {
            if (bottle.getID().equals(bottleId)) {
                return bottle;
            }
        }
        return null;
    }

    public void use(Adventurer target, String itemID) { //bottle or spell
        if (this.hitPoint == 0) { //user is dead
            System.out.printf("%s is dead!\n", this.id);
        }
        else if (target.hitPoint == 0) { //target is dead
            System.out.printf("%s is dead!\n", target.id);
        }
        else {
            int prevHp = target.hitPoint;
            Usable foundUsable = fdUsable(itemID);
            if (foundUsable != null) { // 在包里
                foundUsable.use(this, target);
            }
            else { // 不在包里
                Bottle foundBottle = fdBottle(itemID); // 一定存在
                if (!(this.getMasters().contains(target)
                        || this.getServants().contains(target)
                        || target.equals(this))) { // 不是盟友
                    System.out.print("That's not my ally\n");
                }
                else {
                    System.out.printf("%s fail to use %s\n", this.getID(), itemID);
                }
            }
            target.checkHelp(prevHp);
        }
    }

    public void fight(Adventurer target, int totalDef, int totalNum, int index) {
        if (this.hitPoint == 0) {
            System.out.printf("%s is dead!\n", this.id);
        }
        else if (target.hitPoint == 0) {
            System.out.printf("%s is dead!\n", target.id);
        }
        else {
            if (this.myWeapon == null || this.myWeapon.getClassName().equals("Sword")) {
                int tmpAtk = 0;
                if (this.myWeapon != null) { tmpAtk = this.myWeapon.getCe(); }
                if (this.atk + tmpAtk <= totalDef) {
                    if (index == totalNum) {
                        System.out.printf("Adventurer %s defeated\n", this.id);
                    }
                }
                else {
                    target.underAttack(this.atk + tmpAtk -  totalDef);
                    if (target.getHitPoint() == 0) { this.addMoney(earnMoney(target)); } //得到所有钱
                    if (totalNum == index) { System.out.printf("%d\n", target.getHitPoint()); }
                    else { System.out.printf("%d ", target.getHitPoint()); }
                }
            }
            else if (this.myWeapon.getClassName().equals("Magicbook")) {
                if (this.getMana() >= (int)Math.ceil(Math.sqrt(this.myWeapon.getCe()))) {
                    target.underAttack(this.atk + this.myWeapon.getCe());
                    if (target.getHitPoint() == 0) { this.addMoney(earnMoney(target)); } //得到所有钱
                    if (index == totalNum) { //只要扣一次蓝
                        this.addMana(-(int)Math.ceil(Math.sqrt(this.myWeapon.getCe())));
                    }
                    if (totalNum == index) { System.out.printf("%d\n", target.getHitPoint()); }
                    else { System.out.printf("%d ", target.getHitPoint()); }
                }
                else {
                    if (index == totalNum) {
                        System.out.printf("Adventurer %s defeated\n", this.id);
                    }
                }
            }
        }
    }

    public int earnMoney(Adventurer target) {
        int rewards = 0;
        int bottleEf = 0;
        for (Bottle bottle : target.bottles) {
            bottleEf += bottle.getEffect();
        }
        int equipmentEf = 0;
        for (Equipment equipment : target.equipments) {
            equipmentEf += equipment.getCe();
        }
        rewards = target.getMoney() + bottleEf + equipmentEf;
        return  rewards;
    }

    public void buy(String id, String type) {
        if (this.hitPoint == 0) {
            System.out.printf("%s is dead!\n", this.id);
        }
        else {
            Factory.crtItem(this, id, type);
            System.out.printf("%d\n", this.getMoney());
        }
    }

    @Override
    public void addServant(Adventurer servant) { // 作为上级添加下级
        if (this.hitPoint == 0) { System.out.printf("%s is dead!\n", this.id); }
        else if (servant.getHitPoint() == 0) { System.out.printf("%s is dead!\n", servant.id); }
        else {
            if (this.getServants().contains(servant)) { return; }
            this.servants.add(servant); // 自己添加 servant 为下级
            servant.getMasters().add(this); // servant 添加自己为上级
            for (Adventurer serv : servant.getServants()) {
                this.getServants().add(serv); // 自己添加 servant 的全部下级为下级
                serv.getMasters().add(this); // servant 的全部下级添加自己为上级
            }
            for (Adventurer mast : this.masters) {
                mast.getServants().add(servant); // 全部自己的上级添加 servant 为下级
                servant.getMasters().add(mast); // servant 添加自己的全部上级为上级
                for (Adventurer serv : servant.getServants()) {
                    serv.getMasters().add(mast); // servant 的全部下級添加自己的全部上级为上级
                    mast.getServants().add(serv); // 自己的全部上级添加 servant 的全部下级为下级
                }
            }
        }
    }

    @Override
    public void removeServant(Adventurer servant) {
        if (this.hitPoint == 0) { System.out.printf("%s is dead!\n", this.id); }
        else if (servant.getHitPoint() == 0) { System.out.printf("%s is dead!\n", servant.id); }
        else {
            this.getServants().remove(servant); // 自己的下级移除 servant
            servant.getMasters().remove(this); // servant 的上级移除自己
            for (Adventurer serv : servant.getServants()) {
                this.getServants().remove(serv); // 自己的下级移除 servant 的全部下级
                serv.getMasters().remove(this); // servant 的全部下级的上级移除自己
            }
            for (Adventurer mast : this.masters) {
                mast.getServants().remove(servant); // 自己的全部上级的下级移除 servant
                servant.getMasters().remove(mast); // servant 的上级移除自己的全部上级
                for (Adventurer serv : servant.getServants()) {
                    serv.getMasters().remove(mast); // servant 的全部下级的上级移除自己的全部上级
                    mast.getServants().remove(serv); // 自己的全部上级的下级移除 servant 的全部下级
                }
            }
        }
    }

    @Override
    public boolean helpMaster(Adventurer master) { // 作为下级治疗上级，上级一定活，自己一定活，成功则 true
        ArrayList<Spell> s = new ArrayList<>(); // 魔力足以释放的，且学习过的治疗法术记为一个集合 S
        for (Spell spell : this.spells) {
            if (spell.getClassName().equals("HealSpell") && this.mana >= spell.getManaCost()) {
                s.add(spell);
            }
        }
        if (s.isEmpty()) { return false; } // 失败
        else {
            Spell toUse = null;
            int maxPower = 0;
            for (Spell spell : s) {
                if (spell.getPower() > maxPower) {
                    maxPower = spell.getPower();
                    toUse = spell;
                }
                else if (toUse != null && spell.getPower() == maxPower) {
                    if (spell.getManaCost() < toUse.getManaCost()) { toUse = spell; }
                }
            }
            this.addMana(-toUse.getManaCost());
            master.addHitPoint(toUse.getPower());
            return true;
        }
    }

    @Override
    public void helpMe() { // 作为上级要求治疗
        if (!this.servants.isEmpty()) {
            int cnt = 0;
            for (Adventurer servant : this.servants) {
                if (servant.helpMaster(this)) { cnt++; }
            }
            if (cnt != 0) {
                System.out.printf("%s is helped by %d adventurer(s), now Hp is %d\n",
                        this.id, cnt, this.hitPoint);
            }
        }
    }

    public static Adventurer findAdv(ArrayList<Adventurer> adventurers, String advId) { //输入一个ID，返回之
        for (Adventurer adv : adventurers) {
            if (advId.equals(adv.getID())) {
                return adv;
            }
        }
        return null;
    }
}
